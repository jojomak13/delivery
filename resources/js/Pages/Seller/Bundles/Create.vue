...
const save = () => {
    form.post(route('seller.bundles.store'))
}

const newProduct = () => {
    form.products.push({id: new Date().getTime(), product: ''})
}
...
const search = (search, loading) => {
    loading(true)
    if(search){
        Inertia.get(route('seller.bundles.create'), {search}, {
            preserveState: true,
            replace: true,
...
<template>
    <Head :title="t('app.bundle.create')"/>
