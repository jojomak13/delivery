...
const deleteProduct = (product) => {
    ...
    Inertia.delete(route('seller.products.destroy', product))
}
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                ...
                <Link type="primary" :href="route('seller.products.create')">{{ t('app.product.create') }}</Link>
            </div>
        </template>
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <template v-if="products.data.length">
                                <tr v-for="product in products.data" :key="product.id" :class="{'bg-red-50': !product.approved}">
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        <span v-if="product.available" class="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">Available</span>
                                    ...
                                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                        <InertiaLink :href="route('seller.products.edit', product)" class="text-indigo-600 hover:text-indigo-900">
                                            {{ t('app.edit') }}
                                        </InertiaLink> |
