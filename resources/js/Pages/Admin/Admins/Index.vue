...
const deleteadmin = (admin) => {
    ...
    Inertia.delete(route('admin.admins.destroy', admin))
}
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                ...
                <Link type="primary" :href="route('admin.admins.create')">{{ t('app.admin.create') }}</Link>
            </div>
        </template>
                            ...
                            <template v-if="admins.data.length">
                                <tr v-for="admin in admins.data" :key="admin.id">
                                    ...
                                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                        <InertiaLink :href="route('admin.admins.edit', admin)" class="text-indigo-600 hover:text-indigo-900">
                                            {{ t('app.edit') }}
                                        </InertiaLink> |
                                        <button @click="deleteadmin(admin)" class="text-red-600 hover:text-red-900">
                                            {{ t('app.delete') }}
                                        </button>
                                    </td>
                                </tr>
