...
const deleteProduct = (bundle) => {
    ...
    Inertia.delete(route('admin.bundles.destroy', bundle))
}
</script>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <template v-if="bundles.data.length">
                                <tr v-for="bundle in bundles.data" :key="bundle.id">
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        <span v-if="bundle.approved" class="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">{{ t('app.active') }}</span>
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        <div class="text-gray-900">{{ bundle.store.name }}</div>
                                    </td>
                                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                        <InertiaLink :href="route('admin.bundles.edit', bundle)" class="text-indigo-600 hover:text-indigo-900">
                                            {{ t('app.edit') }}
                                        </InertiaLink> |
